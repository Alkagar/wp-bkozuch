	...
	foreach ($options as $value) {
		...
		case ($value['type'] == 'select'):
			...
			foreach ($value['options'] as $key => $option ) {
				$selected = '';
				 if( $val != '' ) {
					 if ( $val == $key) { $selected = selected($val, $key);} 
			     }
		...
		// Uploader
		case "upload":
			$output .= firstyme_medialibrary_uploader( $value['id'], $val, null ); // New AJAX Uploader using Media Library	
		break;
		...
		case 'background':
			...
			$output .= firstyme_medialibrary_uploader( $value['id'], $background['image'], null, '',0,'image');
