...
if ( is_admin() ) {
	...
	add_action( "admin_print_styles-$of_page", 'firstyme_mlu_css', 0 );
	add_action( "admin_print_scripts-$of_page", 'firstyme_mlu_js', 0 );	
}
...
if ( ! function_exists( 'firstyme_mlu_init' ) ) {
	function firstyme_mlu_init () {
		register_post_type( 'firstyme', array(
			'labels' => array(
				'name' => __( 'Options Framework Internal Container', 'firstyme' ),
			),
			'public' => true,
			'show_ui' => false,
...
if ( ! function_exists( 'firstyme_mlu_css' ) ) {

	function firstyme_mlu_css () {
...
if ( ! function_exists( 'firstyme_mlu_js' ) ) {

	function firstyme_mlu_js () {
 ...
 *
 * Dependencies:
 * - firstyme_mlu_get_silentpost()
 */

if ( ! function_exists( 'firstyme_medialibrary_uploader' ) ) {

	function firstyme_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
		...
		$int = firstyme_mlu_get_silentpost( $id );
		...
		$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . __( 'Upload', 'firstyme' ) . '" rel="' . $int . '" />' . "\n";
 ...
 * Return: $_postid.
 *
 * If no "silent" post is present, one will be created with the type "firstyme"
 * and the post_name of "of-$_token".
 *
 * Example Usage:
 * firstyme_mlu_get_silentpost ( 'of_logo' );
 */

if ( ! function_exists( 'firstyme_mlu_get_silentpost' ) ) {

	function firstyme_mlu_get_silentpost ( $_token ) {
		...
		if ( $_token ) {
			...
			$_args = array( 'post_type' => 'firstyme', 'post_name' => 'of-' . $_token, 'post_status' => 'draft', 'comment_status' => 'closed', 'ping_status' => 'closed' );
...
if ( ! function_exists( 'firstyme_mlu_insidepopup' ) ) {

	function firstyme_mlu_insidepopup () {
	
		if ( isset( $_REQUEST['is_firstyme'] ) && $_REQUEST['is_firstyme'] == 'yes' ) {
		
			add_action( 'admin_head', 'firstyme_mlu_js_popup' );
			add_filter( 'media_upload_tabs', 'firstyme_mlu_modify_tabs' );
		}
	}
}

if ( ! function_exists( 'firstyme_mlu_js_popup' ) ) {

	function firstyme_mlu_js_popup () {
...
?>
	...
	jQuery(function($) {
		...
		// Preserve the "is_firstyme" parameter on the "delete" confirmation button.
		$( '.savesend a.del-link' ).click ( function () {
			...
			continueHref = continueHref + '&is_firstyme=yes';
...
if ( ! function_exists( 'firstyme_mlu_modify_tabs' ) ) {

	function firstyme_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( __( 'Gallery', 'firstyme' ), __( 'Previously Uploaded', 'firstyme' ), $tabs['gallery'] );
		return $tabs;
	}
}